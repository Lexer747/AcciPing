...
func (t *Terminal) StartRaw(
	...
	t.cleanup = func() {
		if err := recover(); err != nil {
			// If the terminal panics we must return back the terminal to a normal state so that other go
...
func (t *Terminal) processListenedRune(r rune) {
	runFallback := true
	f, err := os.OpenFile("/home/lexer747/repos/AcciPing/tmp", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0o777)
	check.NoErr(err, "tmp")
	defer f.Close()
	fmt.Fprintf(f, "Input %+v %s\n", strconv.QuoteRune(r), time.Now().String())
	for _, l := range t.listeners {
		if !l.Applicable(r) {
			continue
		}
		fmt.Fprintf(f, "Running %+v %s\n", l.Name, time.Now().String())
		err := l.Action(r)
		if err != nil {
			panic(errors.Wrapf(err, "unexpected failure Action %q in terminal", l.Name))
		}
		fmt.Fprintf(f, "Completed %+v %s\n", l.Name, time.Now().String())
		runFallback = false
	}
	fmt.Fprintf(f, "Fallback %s %s\n", strconv.QuoteRune(r), time.Now().String())
	if runFallback {
